...
/**
 * Base class that represents a query for the '' . _DB_PREFIX_ . 'djland_connections_page' table.
 *
 * 
 *
...
abstract class Oops_Model_Base_ConnectionsPageQuery extends ModelCriteria
{
	...
	{
		$sql = 'SELECT `ID_CONNECTIONS`, `ID_PAGE`, `TIME_START`, `TIME_END` FROM `' . _DB_PREFIX_ . 'djland_connections_page` WHERE `ID_CONNECTIONS` = :p0 AND `ID_PAGE` = :p1 AND `TIME_START` = :p2';
		try {
