...
/**
 * Base class that represents a row from the 'djland_order_slip' table.
 *
 * 
 *
 * @package    propel.generator.prestashop.om
 */
abstract class Oops_Model_Base_OrderSlip extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'Oops_Model_OrderSlipPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        Oops_Model_OrderSlipPeer
	 */
	...
	/**
	 * Initializes internal state of Oops_Model_Base_OrderSlip object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Oops_Model_OrderSlip The current object (for fluent API support)
	 */
	public function setIdOrderSlip($v)
	{
		...
		if ($this->id_order_slip !== $v) {
			...
			$this->modifiedColumns[] = Oops_Model_OrderSlipPeer::ID_ORDER_SLIP;
		}
	...
	public function setConversionRate($v)
	{
		...
		if ($this->conversion_rate !== $v) {
			...
			$this->modifiedColumns[] = Oops_Model_OrderSlipPeer::CONVERSION_RATE;
		}
	...
	public function setIdCustomer($v)
	{
		...
		if ($this->id_customer !== $v) {
			$this->id_customer = $v;
			$this->modifiedColumns[] = Oops_Model_OrderSlipPeer::ID_CUSTOMER;
		}
	...
	public function setIdOrder($v)
	{
		...
		if ($this->id_order !== $v) {
			$this->id_order = $v;
			$this->modifiedColumns[] = Oops_Model_OrderSlipPeer::ID_ORDER;
		}
	...
	public function setShippingCost($v)
	{
		...
		if ($this->shipping_cost !== $v) {
			...
			$this->modifiedColumns[] = Oops_Model_OrderSlipPeer::SHIPPING_COST;
		}
	...
	public function setDateAdd($v)
	{
		...
		if ($this->date_add !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = Oops_Model_OrderSlipPeer::DATE_ADD;
			}
	...
	public function setDateUpd($v)
	{
		...
		if ($this->date_upd !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = Oops_Model_OrderSlipPeer::DATE_UPD;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 7; // 7 = Oops_Model_OrderSlipPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Oops_Model_OrderSlip object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = Oops_Model_OrderSlipPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = Oops_Model_OrderSlipQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				Oops_Model_OrderSlipPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::ID_ORDER_SLIP)) {
			$modifiedColumns[':p' . $index++]  = '`ID_ORDER_SLIP`';
		}
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::CONVERSION_RATE)) {
			$modifiedColumns[':p' . $index++]  = '`CONVERSION_RATE`';
		}
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::ID_CUSTOMER)) {
			$modifiedColumns[':p' . $index++]  = '`ID_CUSTOMER`';
		}
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::ID_ORDER)) {
			$modifiedColumns[':p' . $index++]  = '`ID_ORDER`';
		}
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::SHIPPING_COST)) {
			$modifiedColumns[':p' . $index++]  = '`SHIPPING_COST`';
		}
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::DATE_ADD)) {
			$modifiedColumns[':p' . $index++]  = '`DATE_ADD`';
		}
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::DATE_UPD)) {
			$modifiedColumns[':p' . $index++]  = '`DATE_UPD`';
		}

		$sql = sprintf(
			'INSERT INTO `' . _DB_PREFIX_ . 'djland_order_slip` (%s) VALUES (%s)',
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Oops_Model_OrderSlipPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Oops_Model_OrderSlipPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Oops_Model_OrderSlip'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Oops_Model_OrderSlip'][$this->getPrimaryKey()] = true;
		$keys = Oops_Model_OrderSlipPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::ID_ORDER_SLIP)) $criteria->add(Oops_Model_OrderSlipPeer::ID_ORDER_SLIP, $this->id_order_slip);
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::CONVERSION_RATE)) $criteria->add(Oops_Model_OrderSlipPeer::CONVERSION_RATE, $this->conversion_rate);
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::ID_CUSTOMER)) $criteria->add(Oops_Model_OrderSlipPeer::ID_CUSTOMER, $this->id_customer);
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::ID_ORDER)) $criteria->add(Oops_Model_OrderSlipPeer::ID_ORDER, $this->id_order);
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::SHIPPING_COST)) $criteria->add(Oops_Model_OrderSlipPeer::SHIPPING_COST, $this->shipping_cost);
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::DATE_ADD)) $criteria->add(Oops_Model_OrderSlipPeer::DATE_ADD, $this->date_add);
		if ($this->isColumnModified(Oops_Model_OrderSlipPeer::DATE_UPD)) $criteria->add(Oops_Model_OrderSlipPeer::DATE_UPD, $this->date_upd);
	...
	{
		$criteria = new Criteria(Oops_Model_OrderSlipPeer::DATABASE_NAME);
		$criteria->add(Oops_Model_OrderSlipPeer::ID_ORDER_SLIP, $this->id_order_slip);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Oops_Model_OrderSlip (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Oops_Model_OrderSlip Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     Oops_Model_OrderSlipPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new Oops_Model_OrderSlipPeer();
		}
		return self::$peer;
	}
	...
	{
		return (string) $this->exportTo(Oops_Model_OrderSlipPeer::DEFAULT_STRING_FORMAT);
	}

} // Oops_Model_Base_OrderSlip
