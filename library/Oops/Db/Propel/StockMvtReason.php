 ...
 */
abstract class Oops_Db_Propel_StockMvtReason extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'Oops_Db_StockMvtReasonPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        Oops_Db_StockMvtReasonPeer
	 */
	...
	/**
	 * Initializes internal state of Oops_Db_Propel_StockMvtReason object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Oops_Db_StockMvtReason The current object (for fluent API support)
	 */
	public function setIdStockMvtReason($v)
	{
		...
		if ($this->id_stock_mvt_reason !== $v) {
			...
			$this->modifiedColumns[] = Oops_Db_StockMvtReasonPeer::ID_STOCK_MVT_REASON;
		}
	...
	public function setSign($v)
	{
		...
		if ($this->sign !== $v) {
			$this->sign = $v;
			$this->modifiedColumns[] = Oops_Db_StockMvtReasonPeer::SIGN;
		}
	...
	public function setDateAdd($v)
	{
		...
		if ($this->date_add !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = Oops_Db_StockMvtReasonPeer::DATE_ADD;
			}
	...
	public function setDateUpd($v)
	{
		...
		if ($this->date_upd !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = Oops_Db_StockMvtReasonPeer::DATE_UPD;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = Oops_Db_StockMvtReasonPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Oops_Db_StockMvtReason object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = Oops_Db_StockMvtReasonPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = Oops_Db_StockMvtReasonQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				Oops_Db_StockMvtReasonPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(Oops_Db_StockMvtReasonPeer::ID_STOCK_MVT_REASON)) {
			$modifiedColumns[':p' . $index++]  = '`ID_STOCK_MVT_REASON`';
		}
		if ($this->isColumnModified(Oops_Db_StockMvtReasonPeer::SIGN)) {
			$modifiedColumns[':p' . $index++]  = '`SIGN`';
		}
		if ($this->isColumnModified(Oops_Db_StockMvtReasonPeer::DATE_ADD)) {
			$modifiedColumns[':p' . $index++]  = '`DATE_ADD`';
		}
		if ($this->isColumnModified(Oops_Db_StockMvtReasonPeer::DATE_UPD)) {
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Oops_Db_StockMvtReasonPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Oops_Db_StockMvtReasonPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Oops_Db_StockMvtReason'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Oops_Db_StockMvtReason'][serialize($this->getPrimaryKey())] = true;
		$keys = Oops_Db_StockMvtReasonPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Oops_Db_StockMvtReasonPeer::ID_STOCK_MVT_REASON)) $criteria->add(Oops_Db_StockMvtReasonPeer::ID_STOCK_MVT_REASON, $this->id_stock_mvt_reason);
		if ($this->isColumnModified(Oops_Db_StockMvtReasonPeer::SIGN)) $criteria->add(Oops_Db_StockMvtReasonPeer::SIGN, $this->sign);
		if ($this->isColumnModified(Oops_Db_StockMvtReasonPeer::DATE_ADD)) $criteria->add(Oops_Db_StockMvtReasonPeer::DATE_ADD, $this->date_add);
		if ($this->isColumnModified(Oops_Db_StockMvtReasonPeer::DATE_UPD)) $criteria->add(Oops_Db_StockMvtReasonPeer::DATE_UPD, $this->date_upd);
	...
	{
		$criteria = new Criteria(Oops_Db_StockMvtReasonPeer::DATABASE_NAME);
		$criteria->add(Oops_Db_StockMvtReasonPeer::ID_STOCK_MVT_REASON, $this->id_stock_mvt_reason);
		$criteria->add(Oops_Db_StockMvtReasonPeer::SIGN, $this->sign);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Oops_Db_StockMvtReason (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Oops_Db_StockMvtReason Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     Oops_Db_StockMvtReasonPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new Oops_Db_StockMvtReasonPeer();
		}
		return self::$peer;
	}

	/**
	 * Declares an association between this object and a Oops_Db_StockMvt object.
	 *
	 * @param      Oops_Db_StockMvt $v
	 * @return     Oops_Db_StockMvtReason The current object (for fluent API support)
	 * @throws     PropelException
	 */
	public function setStockMvt(Oops_Db_StockMvt $v = null)
	{
		if ($v === null) {
		...
		// If this object has already been added to the Oops_Db_StockMvt object, it will not be re-added.
		if ($v !== null) {
	...
	/**
	 * Get the associated Oops_Db_StockMvt object
	 *
	 * @param      PropelPDO Optional Connection object.
	 * @return     Oops_Db_StockMvt The associated Oops_Db_StockMvt object.
	 * @throws     PropelException
	 */
	public function getStockMvt(PropelPDO $con = null)
	{
		if ($this->aStockMvt === null && ($this->id_stock_mvt_reason !== null)) {
			$this->aStockMvt = Oops_Db_StockMvtQuery::create()
	...
	{
		return (string) $this->exportTo(Oops_Db_StockMvtReasonPeer::DEFAULT_STRING_FORMAT);
	}

} // Oops_Db_Propel_StockMvtReason
