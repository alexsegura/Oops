 ...
 */
abstract class Oops_Db_Propel_Log extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'Oops_Db_LogPeer';
	 ...
	 * @return     Oops_Db_Log The current object (for fluent API support)
	 */
	public function setIdLog($v)
	{
		...
		if ($this->id_log !== $v) {
			$this->id_log = $v;
			$this->modifiedColumns[] = Oops_Db_LogPeer::ID_LOG;
		}
	...
	public function setSeverity($v)
	{
		...
		if ($this->severity !== $v) {
			$this->severity = $v;
			$this->modifiedColumns[] = Oops_Db_LogPeer::SEVERITY;
		}
	...
	public function setErrorCode($v)
	{
		...
		if ($this->error_code !== $v) {
			$this->error_code = $v;
			$this->modifiedColumns[] = Oops_Db_LogPeer::ERROR_CODE;
		}
	...
	public function setMessage($v)
	{
		...
		if ($this->message !== $v) {
			$this->message = $v;
			$this->modifiedColumns[] = Oops_Db_LogPeer::MESSAGE;
		}
	...
	public function setObjectType($v)
	{
		...
		if ($this->object_type !== $v) {
			$this->object_type = $v;
			$this->modifiedColumns[] = Oops_Db_LogPeer::OBJECT_TYPE;
		}
	...
	public function setObjectId($v)
	{
		...
		if ($this->object_id !== $v) {
			$this->object_id = $v;
			$this->modifiedColumns[] = Oops_Db_LogPeer::OBJECT_ID;
		}
	...
	public function setDateAdd($v)
	{
		...
		if ($this->date_add !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = Oops_Db_LogPeer::DATE_ADD;
			}
	...
	public function setDateUpd($v)
	{
		...
		if ($this->date_upd !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = Oops_Db_LogPeer::DATE_UPD;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 8; // 8 = Oops_Db_LogPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Oops_Db_Log object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = Oops_Db_LogPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = Oops_Db_LogQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				Oops_Db_LogPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(Oops_Db_LogPeer::ID_LOG)) {
			$modifiedColumns[':p' . $index++]  = '`ID_LOG`';
		}
		if ($this->isColumnModified(Oops_Db_LogPeer::SEVERITY)) {
			$modifiedColumns[':p' . $index++]  = '`SEVERITY`';
		}
		if ($this->isColumnModified(Oops_Db_LogPeer::ERROR_CODE)) {
			$modifiedColumns[':p' . $index++]  = '`ERROR_CODE`';
		}
		if ($this->isColumnModified(Oops_Db_LogPeer::MESSAGE)) {
			$modifiedColumns[':p' . $index++]  = '`MESSAGE`';
		}
		if ($this->isColumnModified(Oops_Db_LogPeer::OBJECT_TYPE)) {
			$modifiedColumns[':p' . $index++]  = '`OBJECT_TYPE`';
		}
		if ($this->isColumnModified(Oops_Db_LogPeer::OBJECT_ID)) {
			$modifiedColumns[':p' . $index++]  = '`OBJECT_ID`';
		}
		if ($this->isColumnModified(Oops_Db_LogPeer::DATE_ADD)) {
			$modifiedColumns[':p' . $index++]  = '`DATE_ADD`';
		}
		if ($this->isColumnModified(Oops_Db_LogPeer::DATE_UPD)) {
			$modifiedColumns[':p' . $index++]  = '`DATE_UPD`';
		}

		$sql = sprintf(
			'INSERT INTO `' .  _DB_PREFIX_ . 'log` (%s) VALUES (%s)',
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Oops_Db_LogPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Oops_Db_LogPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Oops_Db_Log'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Oops_Db_Log'][$this->getPrimaryKey()] = true;
		$keys = Oops_Db_LogPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Oops_Db_LogPeer::ID_LOG)) $criteria->add(Oops_Db_LogPeer::ID_LOG, $this->id_log);
		if ($this->isColumnModified(Oops_Db_LogPeer::SEVERITY)) $criteria->add(Oops_Db_LogPeer::SEVERITY, $this->severity);
		if ($this->isColumnModified(Oops_Db_LogPeer::ERROR_CODE)) $criteria->add(Oops_Db_LogPeer::ERROR_CODE, $this->error_code);
		if ($this->isColumnModified(Oops_Db_LogPeer::MESSAGE)) $criteria->add(Oops_Db_LogPeer::MESSAGE, $this->message);
		if ($this->isColumnModified(Oops_Db_LogPeer::OBJECT_TYPE)) $criteria->add(Oops_Db_LogPeer::OBJECT_TYPE, $this->object_type);
		if ($this->isColumnModified(Oops_Db_LogPeer::OBJECT_ID)) $criteria->add(Oops_Db_LogPeer::OBJECT_ID, $this->object_id);
		if ($this->isColumnModified(Oops_Db_LogPeer::DATE_ADD)) $criteria->add(Oops_Db_LogPeer::DATE_ADD, $this->date_add);
		if ($this->isColumnModified(Oops_Db_LogPeer::DATE_UPD)) $criteria->add(Oops_Db_LogPeer::DATE_UPD, $this->date_upd);
	...
	{
		$criteria = new Criteria(Oops_Db_LogPeer::DATABASE_NAME);
		$criteria->add(Oops_Db_LogPeer::ID_LOG, $this->id_log);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Oops_Db_Log (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Oops_Db_Log Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     Oops_Db_LogPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new Oops_Db_LogPeer();
		}
		return self::$peer;
	}
	...
	{
		return (string) $this->exportTo(Oops_Db_LogPeer::DEFAULT_STRING_FORMAT);
	}
