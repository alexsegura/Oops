 ...
 */
abstract class Oops_Model_Base_Category extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'Oops_Model_CategoryPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        Oops_Model_CategoryPeer
	 */
	...
	/**
	 * @var        array Oops_Model_Lang[] Collection to store aggregation of Oops_Model_Lang objects.
	 */
	protected $collLangs;
	 ...
	 * @var        array[Oops_Model_CategoryLang]
	 */
	...
	/**
	 * Initializes internal state of Oops_Model_Base_Category object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Oops_Model_Category The current object (for fluent API support)
	 */
	public function setIdCategory($v)
	{
		...
		if ($this->id_category !== $v) {
			$this->id_category = $v;
			$this->modifiedColumns[] = Oops_Model_CategoryPeer::ID_CATEGORY;
		}
	...
	public function setIdParent($v)
	{
		...
		if ($this->id_parent !== $v) {
			$this->id_parent = $v;
			$this->modifiedColumns[] = Oops_Model_CategoryPeer::ID_PARENT;
		}
	...
	public function setLevelDepth($v)
	{
		...
		if ($this->level_depth !== $v) {
			$this->level_depth = $v;
			$this->modifiedColumns[] = Oops_Model_CategoryPeer::LEVEL_DEPTH;
		}
	...
	public function setNleft($v)
	{
		...
		if ($this->nleft !== $v) {
			$this->nleft = $v;
			$this->modifiedColumns[] = Oops_Model_CategoryPeer::NLEFT;
		}
	...
	public function setNright($v)
	{
		...
		if ($this->nright !== $v) {
			$this->nright = $v;
			$this->modifiedColumns[] = Oops_Model_CategoryPeer::NRIGHT;
		}
	...
	public function setActive($v)
	{
		...
		if ($this->active !== $v) {
			$this->active = $v;
			$this->modifiedColumns[] = Oops_Model_CategoryPeer::ACTIVE;
		}
	...
	public function setDateAdd($v)
	{
		...
		if ($this->date_add !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = Oops_Model_CategoryPeer::DATE_ADD;
			}
	...
	public function setDateUpd($v)
	{
		...
		if ($this->date_upd !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = Oops_Model_CategoryPeer::DATE_UPD;
			}
		} // if either are not null
	...
	public function setPosition($v)
	{
		...
		if ($this->position !== $v) {
			$this->position = $v;
			$this->modifiedColumns[] = Oops_Model_CategoryPeer::POSITION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 9; // 9 = Oops_Model_CategoryPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Oops_Model_Category object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = Oops_Model_CategoryPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = Oops_Model_CategoryQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// nested_set behavior
			if ($this->isRoot()) {
				throw new PropelException('Deletion of a root node is disabled for nested sets. Use Oops_Model_CategoryPeer::deleteTree() instead to delete an entire tree');
			}
			...
			if ($ret) {
				...
				if ($this->isInTree()) {
					...
					Oops_Model_CategoryPeer::shiftRLValues(-2, $this->getRightValue() + 1, null, $con);
				}
				...
				Oops_Model_CategoryLangQuery::create()
					->filterByOops_Model_Category($this)
					->delete($con);
				$con->commit();
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($this->isNew() && $this->isRoot()) {
				...
				$nbRoots = Oops_Model_CategoryQuery::create()
			...
			if ($ret) {
				...
				$this->postSave($con);
				Oops_Model_CategoryPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(Oops_Model_CategoryPeer::ID_CATEGORY)) {
			$modifiedColumns[':p' . $index++]  = '`ID_CATEGORY`';
		}
		if ($this->isColumnModified(Oops_Model_CategoryPeer::ID_PARENT)) {
			$modifiedColumns[':p' . $index++]  = '`ID_PARENT`';
		}
		if ($this->isColumnModified(Oops_Model_CategoryPeer::LEVEL_DEPTH)) {
			$modifiedColumns[':p' . $index++]  = '`LEVEL_DEPTH`';
		}
		if ($this->isColumnModified(Oops_Model_CategoryPeer::NLEFT)) {
			$modifiedColumns[':p' . $index++]  = '`NLEFT`';
		}
		if ($this->isColumnModified(Oops_Model_CategoryPeer::NRIGHT)) {
			$modifiedColumns[':p' . $index++]  = '`NRIGHT`';
		}
		if ($this->isColumnModified(Oops_Model_CategoryPeer::ACTIVE)) {
			$modifiedColumns[':p' . $index++]  = '`ACTIVE`';
		}
		if ($this->isColumnModified(Oops_Model_CategoryPeer::DATE_ADD)) {
			$modifiedColumns[':p' . $index++]  = '`DATE_ADD`';
		}
		if ($this->isColumnModified(Oops_Model_CategoryPeer::DATE_UPD)) {
			$modifiedColumns[':p' . $index++]  = '`DATE_UPD`';
		}
		if ($this->isColumnModified(Oops_Model_CategoryPeer::POSITION)) {
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Oops_Model_CategoryPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Oops_Model_CategoryPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Oops_Model_Category'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Oops_Model_Category'][$this->getPrimaryKey()] = true;
		$keys = Oops_Model_CategoryPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Oops_Model_CategoryPeer::ID_CATEGORY)) $criteria->add(Oops_Model_CategoryPeer::ID_CATEGORY, $this->id_category);
		if ($this->isColumnModified(Oops_Model_CategoryPeer::ID_PARENT)) $criteria->add(Oops_Model_CategoryPeer::ID_PARENT, $this->id_parent);
		if ($this->isColumnModified(Oops_Model_CategoryPeer::LEVEL_DEPTH)) $criteria->add(Oops_Model_CategoryPeer::LEVEL_DEPTH, $this->level_depth);
		if ($this->isColumnModified(Oops_Model_CategoryPeer::NLEFT)) $criteria->add(Oops_Model_CategoryPeer::NLEFT, $this->nleft);
		if ($this->isColumnModified(Oops_Model_CategoryPeer::NRIGHT)) $criteria->add(Oops_Model_CategoryPeer::NRIGHT, $this->nright);
		if ($this->isColumnModified(Oops_Model_CategoryPeer::ACTIVE)) $criteria->add(Oops_Model_CategoryPeer::ACTIVE, $this->active);
		if ($this->isColumnModified(Oops_Model_CategoryPeer::DATE_ADD)) $criteria->add(Oops_Model_CategoryPeer::DATE_ADD, $this->date_add);
		if ($this->isColumnModified(Oops_Model_CategoryPeer::DATE_UPD)) $criteria->add(Oops_Model_CategoryPeer::DATE_UPD, $this->date_upd);
		if ($this->isColumnModified(Oops_Model_CategoryPeer::POSITION)) $criteria->add(Oops_Model_CategoryPeer::POSITION, $this->position);
	...
	{
		$criteria = new Criteria(Oops_Model_CategoryPeer::DATABASE_NAME);
		$criteria->add(Oops_Model_CategoryPeer::ID_CATEGORY, $this->id_category);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Oops_Model_Category (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Oops_Model_Category Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     Oops_Model_CategoryPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new Oops_Model_CategoryPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this Oops_Model_Category is new, it will return
	...
	public function initLangs()
	{
		...
		$this->collLangs->setModel('Oops_Model_Lang');
	}

	/**
	 * Gets a collection of Oops_Model_Lang objects related by a many-to-many relationship
	 ...
	 * @param      PropelPDO $con Optional connection object
	 *
	 * @return     PropelCollection|array Oops_Model_Lang[] List of Oops_Model_Lang objects
	 */
	public function getLangs($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collLangs || null !== $criteria) {
			if ($this->isNew() && null === $this->collLangs) {
				...
				$collLangs = Oops_Model_LangQuery::create(null, $criteria)
					->filterByCategory($this)
					->find($con);
				if (null !== $criteria) {
					return $collLangs;
	...
	/**
	 * Sets a collection of Oops_Model_Lang objects related by a many-to-many relationship
	 * to the current object by way of the category_lang cross-reference table.
	 * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
	 * and new objects from the given Propel collection.
	 *
	...
	public function setLangs(PropelCollection $langs, PropelPDO $con = null)
	{
		$oops_Model_CategoryLangs = CategoryLangQuery::create()
			->filterByOops_Model_Lang($langs)
			->filterByCategory($this)
			->find($con);

		$this->langsScheduledForDeletion = $this->getCategoryLangs()->diff($oops_Model_CategoryLangs);
		$this->collCategoryLangs = $oops_Model_CategoryLangs;

		foreach ($langs as $lang) {
			// Fix issue with collection modified by reference
			if ($lang->isNew()) {
				$this->doAddOops_Model_Lang($lang);
			} else {
				$this->addOops_Model_Lang($lang);
			}
		}
	...
	/**
	 * Gets the number of Oops_Model_Lang objects related by a many-to-many relationship
	 * to the current object by way of the category_lang cross-reference table.
	 *
	 * @param      Criteria $criteria Optional query object to filter the query
	 ...
	 * @param      PropelPDO $con Optional connection object
	 *
	 * @return     int the number of related Oops_Model_Lang objects
	 */
	public function countLangs($criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collLangs || null !== $criteria) {
			if ($this->isNew() && null === $this->collLangs) {
				...
				$query = Oops_Model_LangQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
	...
	/**
	 * Associate a Oops_Model_Lang object to this object
	 * through the category_lang cross reference table.
	 *
	 * @param      Oops_Model_Lang $lang The Oops_Model_CategoryLang object to relate
	 * @return     void
	 */
	...
	public function setProducts(PropelCollection $products, PropelPDO $con = null)
	{
		...
		$this->productsScheduledForDeletion = $this->getCategoryProducts()->diff($oops_Model_CategoryProducts);
	 ...
	 *
	 * @param      Oops_Model_Product $product The Oops_Model_CategoryProduct object to relate
	 * @return     void
	 */
	...
	{
		return (string) $this->exportTo(Oops_Model_CategoryPeer::DEFAULT_STRING_FORMAT);
	}
	 ...
	 *
	 * @param      Oops_Model_Category $node Propel node object
	 * @return     bool
	 */
	 ...
	 *
	 * @param      Oops_Model_Category $parent
	 * @return     Oops_Model_Category The current object, for fluid interface
	 */
	...
	public function getParent(PropelPDO $con = null)
	{
		if ($this->aNestedSetParent === null && $this->hasParent()) {
			$this->aNestedSetParent = Oops_Model_CategoryQuery::create()
				->ancestorsOf($this)
				->orderByLevel(true)
				->findOne($con);
		}
	...
	{
		if (!Oops_Model_CategoryPeer::isValid($this)) {
			return false;
		}
		return Oops_Model_CategoryQuery::create()
	...
	public function initNestedSetChildren()
	{
		...
		$this->collNestedSetChildren->setModel('Oops_Model_Category');
	}
	 ...
	 *
	 * @param      Oops_Model_Category $category
	 *
	 * @return     void
	 */
	 ...
	 * @param      PropelPDO $con Connection to use.
	 * @return     array     List of Oops_Model_Category objects
	 */
	public function getChildren($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collNestedSetChildren || null !== $criteria) {
			if ($this->isLeaf() || ($this->isNew() && null === $this->collNestedSetChildren)) {
				...
				$collNestedSetChildren = Oops_Model_CategoryQuery::create(null, $criteria)
	  			->childrenOf($this)
	  			->orderByBranch()
					->find($con);
				if (null !== $criteria) {
	...
	public function countChildren($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collNestedSetChildren || null !== $criteria) {
			if ($this->isLeaf() || ($this->isNew() && null === $this->collNestedSetChildren)) {
				...
				return Oops_Model_CategoryQuery::create(null, $criteria)
					->childrenOf($this)
					->count($con);
			}
		} else {
	...
	public function getFirstChild($query = null, PropelPDO $con = null)
	{
		if($this->isLeaf()) {
			...
			return Oops_Model_CategoryQuery::create(null, $query)
				->childrenOf($this)
				->orderByBranch()
				->findOne($con);
		}
	...
	public function getSiblings($includeNode = false, $query = null, PropelPDO $con = null)
	{
		if($this->isRoot()) {
			return array();
			 ...
			 $query = Oops_Model_CategoryQuery::create(null, $query)
	 ...
	 *
	 * @param      Oops_Model_Category $child	Propel object for child node
	 *
	 * @return     Oops_Model_Category The current Propel object
	 */
	public function addChild(Oops_Model_Category $child)
	{
		if ($this->isNew()) {
			throw new PropelException('A Oops_Model_Category object must not be new to accept children.');
		}
	 ...
	 *
	 * @param      Oops_Model_Category $parent	Propel object for parent node
	 *
	 * @return     Oops_Model_Category The current Propel object
	 */
	...
	{
		if ($this->isInTree()) {
			throw new PropelException('A Oops_Model_Category object must not already be in the tree to be inserted. Use the moveToFirstChildOf() instead.');
		}
		...
		$this->nestedSetQueries []= array(
			'callable'  => array('Oops_Model_CategoryPeer', 'makeRoomForLeaf'),
	...
	{
		if ($this->isInTree()) {
			throw new PropelException('A Oops_Model_Category object must not already be in the tree to be inserted. Use the moveToLastChildOf() instead.');
		}
	 ...
	 * are not persisted until the current object is saved.
	 *
	 * @param      Oops_Model_Category $sibling	Propel object for parent node
	 *
	 * @return     Oops_Model_Category The current Propel object
	 */
	...
	{
		if ($this->isInTree()) {
			throw new PropelException('A Oops_Model_Category object must not already be in the tree to be inserted. Use the moveToPrevSiblingOf() instead.');
		}
	...
	{
		if ($this->isInTree()) {
			throw new PropelException('A Oops_Model_Category object must not already be in the tree to be inserted. Use the moveToNextSiblingOf() instead.');
		}
	...
	{
		if (!$this->isInTree()) {
			throw new PropelException('A Oops_Model_Category object must be already in the tree to be moved. Use the insertAsFirstChildOf() instead.');
		}
	...
	{
		if (!$this->isInTree()) {
			throw new PropelException('A Oops_Model_Category object must be already in the tree to be moved. Use the insertAsLastChildOf() instead.');
		}
		if ($parent->isDescendantOf($this)) {
			throw new PropelException('Cannot move a node as child of one of its subtree nodes.');
		}
	 ...
	 * The modifications in the current object and the tree are immediate
	 *
	 * @param      Oops_Model_Category $sibling	Propel object for sibling node
	 * @param      PropelPDO $con	Connection to use.
	 *
	 * @return     Oops_Model_Category The current Propel object
	 */
	...
	{
		if (!$this->isInTree()) {
			throw new PropelException('A Oops_Model_Category object must be already in the tree to be moved. Use the insertAsPrevSiblingOf() instead.');
		}
		if ($sibling->isRoot()) {
	...
	{
		if (!$this->isInTree()) {
			throw new PropelException('A Oops_Model_Category object must be already in the tree to be moved. Use the insertAsNextSiblingOf() instead.');
		}
		if ($sibling->isRoot()) {
	...
	protected function moveSubtreeTo($destLeft, $levelDelta, PropelPDO $con = null)
	{
		...
		try {
			...
			Oops_Model_CategoryPeer::shiftRLValues($treeSize, $destLeft, null, $con);
			...
			if ($levelDelta) {
				...
				Oops_Model_CategoryPeer::shiftLevel($levelDelta, $left, $right, $con);
			}
	
			// move the subtree to the target
			Oops_Model_CategoryPeer::shiftRLValues($destLeft - $left, $left, $right, $con);
	
			// remove the empty room at the previous location of the subtree
			Oops_Model_CategoryPeer::shiftRLValues(-$treeSize, $right + 1, null, $con);
	
			// update all loaded nodes
			Oops_Model_CategoryPeer::updateLoadedNodes(null, $con);
	 ...
	 * so existing Oops_Model_Category instances are probably invalid (except for the current one)
	 *
	 * @param      PropelPDO $con Connection to use.
	 *
	...
	public function deleteDescendants(PropelPDO $con = null)
	{
		...
		try {
			...
			$ret = Oops_Model_CategoryQuery::create()
				->descendantsOf($this)
				->delete($con);
	
			// fill up the room that was used by descendants
			Oops_Model_CategoryPeer::shiftRLValues($left - $right + 1, $right, null, $con);
	 ...
	 *
	 * @return Oops_Model_CategoryLang */
	public function getTranslation($locale = '1', PropelPDO $con = null)
	{
		if (!isset($this->currentTranslations[$locale])) {
			...
			if ($this->isNew()) {
				$translation = new Oops_Model_CategoryLang();
				$translation->setIdLang($locale);
			} else {
				$translation = Oops_Model_CategoryLangQuery::create()
